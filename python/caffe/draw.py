    ...
    if layer.type == 'Convolution' or layer.type == 'Deconvolution':
        ...
        node_label = '"%s%s(%s)%skernel size: %d%sstride: %d%spad: %d"' %\
                     (layer.name,
                      ...
                      separator,
                      layer.convolution_param.pad[0] if len(layer.convolution_param.pad._values) else 0) # Modified by ARUN to correct error due to repeated fields
        ...
        node_label = '"%s%s(%s %s)%skernel size: %d%sstride: %d%spad: %d"' %\
                     (layer.name,
                      ...
                      separator,
                      layer.pooling_param.pad[0] if len(layer.pooling_param.pad._values) else 0) # Modified by ARUN to correct error due to repeated fields
